...
FILTERS_FILE  = os.path.join(BASE_DIR, "filters.json")   # { "<chat_id>": { "":[lines] } }
...
lines = [f" Alınan Toplam SMS : {total}"]
...
else SESSION.post(url, data=params or {}, timeout=(3, timeout)) # Düzeltme: 'r = '
...
m = re.search(r'(?i)\bfrom\s*[:=]\s*([^\s,;]+)', content) # REGEX'i biraz düzelttim: birden çok karakteri yakala
if m:
...
raw_brand = extract_from_field(row) # İlk ham gönderici adını/numarasını al
...
brand_key = detect_brand_key(row)  # ör: "google", "facebook", "getir", "vakifbank" ...
